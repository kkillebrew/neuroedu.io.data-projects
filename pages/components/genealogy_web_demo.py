...
def render_genealogy_web():
    ...
    <head>
        ...
        <style>
            ...
            /* LINK STYLES: Thickness Doubled for Data-to-Ink Emphasis */
            .link-main { fill: none; stroke: #475569; stroke-opacity: 0.6; stroke-width: 8px; }
            .link-marriage { fill: none; stroke: #94A3B8; stroke-opacity: 0.3; stroke-width: 4px; stroke-dasharray: 6,6; }
            .link-leaf { fill: none; stroke: #475569; stroke-opacity: 0.8; stroke-width: 3px; }
            .link-inlaw { fill: none; stroke: #64748B; stroke-opacity: 0.6; stroke-width: 3px; stroke-dasharray: 4,4; }
            
            .tree-link { fill: none; stroke: #334155; }
            .tree-link.main { stroke-width: 8px; stroke-opacity: 0.6; }
            .tree-link.leaf { stroke-width: 3px; stroke-opacity: 0.8; }
            .tree-link.inlaw { stroke-width: 3px; stroke-opacity: 0.6; stroke-dasharray: 4,4; }
        ...
        <script>
            ...
            const graphData = {
                nodes: [
                    ...
                    // Killebrew Laterals (from Robert, step 2)
                    ...
                    // Killebrew Laterals (from William H.K., step 3)
                    ...
                    // Rasmussen Laterals (from Clinton, step 3)
                    ...
                    // Rasmussen Laterals (from Richard/Bettie, step 3)
                    { id: "James", branch: "R", steps: 3, lateral: 2 },
                    { id: "Rosemary", branch: "R", steps: 3, lateral: 2 },
                    { id: "Ruth", branch: "R", steps: 3, lateral: 2 },
                    { id: "Karen", branch: "R", steps: 3, lateral: 2 },
                    { id: "Michelle", branch: "R", steps: 3, lateral: 2 },
                    ...
                    { id: "Beulah Salisbury", branch: "V", steps: 5, lateral: 0, inLaw: true }, // Married into bloodline
                    ...
                    { id: "Marie Emilie Ibe", branch: "L", steps: 3, lateral: 0, inLaw: true }, // Married into bloodline
                        ...
                        children: [
                            { name: "Robert Killebrew", year: 1930, branch: "K", steps: 2, lateral: 0, desc: "Grandfather", children: [
                                { name: "William H. K.", year: 1898, branch: "K", steps: 3, lateral: 0, children: [
                                    ...
                                    { name: "Ron K.", year: 1928, branch: "K", steps: 3, lateral: 1 },
                                    { name: "Urma K.", year: 1932, branch: "K", steps: 3, lateral: 1 }
                                ]},
                                { name: "Kelly K.", year: 1955, branch: "K", steps: 2, lateral: 1 },
                                { name: "Stephen K.", year: 1957, branch: "K", steps: 2, lateral: 1 },
                                { name: "Suzie K.", year: 1959, branch: "K", steps: 2, lateral: 1 },
                                { name: "Tony K.", year: 1963, branch: "K", steps: 2, lateral: 1 },
                                { name: "Keri K.", year: 1965, branch: "K", steps: 2, lateral: 1 },
                                { name: "Sheri K.", year: 1967, branch: "K", steps: 2, lateral: 1 }
                            ]},
                            { name: "Bonnie Rasmussen", year: 1934, branch: "R", steps: 2, lateral: 0, desc: "Grandmother", children: [
                                { name: "Clinton R.", year: 1904, branch: "R", steps: 3, lateral: 0, children: [
                                    ...
                                    { name: "Richard R.", year: 1932, branch: "R", steps: 3, lateral: 1, children: [
                                        { name: "Oranell", year: 1932, branch: "R", steps: 3, lateral: 1, inLaw: true },
                                        { name: "James", year: 1955, branch: "R", steps: 3, lateral: 2 },
                                        { name: "Rosemary", year: 1957, branch: "R", steps: 3, lateral: 2 },
                                        { name: "Ruth", year: 1959, branch: "R", steps: 3, lateral: 2 },
                                        { name: "Karen", year: 1961, branch: "R", steps: 3, lateral: 2 }
                                    ]},
                                    { name: "Bettie R.", year: 1936, branch: "R", steps: 3, lateral: 1, children: [
                                        { name: "Bob", year: 1936, branch: "R", steps: 3, lateral: 1, inLaw: true },
                                        { name: "Michelle", year: 1960, branch: "R", steps: 3, lateral: 2 }
                                    ]},
                                    { name: "Rhett R.", year: 1938, branch: "R", steps: 3, lateral: 1 }
                                ]}
                            ]}
                        ]
                    },
                    {
                        ...
                        children: [
                            { name: "John O. Vanderhoop", year: 1934, branch: "V", steps: 2, lateral: 0, desc: "Grandfather", children: [
                                ...
                                { name: "Johnny Vanderhoop", year: 1936, branch: "V", steps: 2, lateral: 1 }
                            ]},
                            { name: "Waltrud M. Lieber", year: 1934, branch: "L", steps: 2, lateral: 0, desc: "Grandmother", children: [
                                { name: "G. Heinrich L. Lieber", year: 1900, branch: "L", steps: 3, lateral: 0, children: [
                                    ...
                                    { name: "Manfred Lieber", year: 1932, branch: "L", steps: 3, lateral: 1 }
                                ]}
                            ]}
                        ]
                    }
            ...
            function calcRadius(d) {
                const ROOT_SIZE = 25;
                if (d.inLaw) return ROOT_SIZE * 0.05; // Spouses are fixed 5%
                ...
                if (d.lateral === 0) {
                    ...
                    // Descendants: 1/3 the size of the parent node they branch from
                    let branchRootSize = ROOT_SIZE * (1 - 0.05 * d.steps);
                    let startSize = branchRootSize / 3;
                    // Decrease by 10% per lateral step
            ...
            function calcColor(d) {
                ...
                if (d.steps > 0) {
                    if (d.branch === "K") { 
                        r = Math.max(0, 240 - 20 * d.steps); // To Blue
                        b = 240;
                    } else if (d.branch === "V") { 
                        r = 240;
                        b = Math.max(0, 240 - 20 * d.steps); // To Red
                    } else if (d.branch === "R") { 
                        let st = d.steps - 1;
                        r = Math.max(0, 220 - 20 * st);
                        b = Math.max(0, 240 - 20 * st);
                        g = Math.min(240, 0 + 20 * st); // To Green
                    } else if (d.branch === "L") { 
                        let st = d.steps - 1;
                        r = 240;
                        b = Math.max(0, 220 - 20 * st);
                        g = Math.min(240, 0 + 20 * st); // To Yellow
                    }
                }

                // Lateral shift +40 towards White relative to parent
                if (d.lateral > 0) {
                    let shift = 40 * d.lateral; 
                    r = Math.min(240, r + shift);
                    g = Math.min(240, g + shift);
                    b = Math.min(240, b + shift);
                }
            ...
            const simulation = d3.forceSimulation(graphData.nodes)
                ...
                .force("charge", d3.forceManyBody().strength(d => d.inLaw ? -15 : -350))
            ...
            legend.append("text").attr("x", 0).attr("y", 125).attr("fill", "#64748B").attr("font-size", "10px").text("Lateral Color: +40 White per step");
            ...
            // Custom separation algorithm for proportional horizontal spacing
            d3.tree().size([tWidth - 60, height])
                .separation((a, b) => {
                    let aLat = (a.data.lateral > 0 || a.data.inLaw);
                    let bLat = (b.data.lateral > 0 || b.data.inLaw);
                    if (aLat && bLat) return 0.15; // Tight sibling packing
                    if (aLat || bLat) return 0.25; // Sibling to main branch packing
                    return 1; // Standard spacing for main branches
                })(root);
            ...
            tNode.append("circle")
                ...
                .on("mouseover", (e,d) => {
                    ...
                    tooltip.html(`<strong>${d.data.name}</strong><br/>${d.data.year} | ${d.data.desc || stat}`)
                        .style("left", (e.pageX + 15) + "px").style("top", (e.pageY - 28) + "px");
                })
                .on("mouseout", () => tooltip.transition().duration(500).style("opacity", 0));
