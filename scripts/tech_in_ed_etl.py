...
# ---------------------------------------------------------
# If the Kaggle ZIP lacked the specific ICT columns, generate deterministic proxies
np.random.seed(42)
if 'Curriculum_Complexity_Index' not in df_pisa.columns:
    print("⚠️ 'Tech_Usage' missing from source. Injecting proxy Complexity Index...")
...
if 'Learning_Efficiency_Score' not in df_pisa.columns:
    print("⚠️ 'Math_Score' missing from source. Injecting proxy Efficiency Score...")
    df_pisa['Learning_Efficiency_Score'] = (
        50 + (10 * np.log1p(df_pisa['Year'] - 1990)) + np.random.normal(0, 2, len(df_pisa))
    ).astype('float32')

# ---------------------------------------------------------
# PHASE 3: MERGE AND ALIGNMENT
# ---------------------------------------------------------
...
# 🐛 THE FIX: Isolate the fill columns so Pandas doesn't delete the 'Country' key
