...
def load_aalto(base_dir):
    """ Loads the pre-processed, lightweight Aalto files generated by the GitHub ETL pipeline. """
    typo_path = os.path.join(base_dir, 'aalto_typos_only.parquet')
    base_path = os.path.join(base_dir, 'aalto_baseline_arrays.parquet')
    
    # Check if the GitHub Action has finished generating the files
    if os.path.exists(typo_path) and os.path.exists(base_path):
        # Load the pre-processed typo footprints (For Tab 1 & 3)
        df_typos = pd.read_parquet(typo_path)
        
        # Load the baseline latencies for the T-Test (For Tab 2)
        df_base = pd.read_parquet(base_path)
        
        # Merge them safely into one dataframe for the frontend router
        df_merged = pd.concat([df_typos, df_base], ignore_index=True)
        return df_merged
    else:
        # Failsafe if the app boots before GitHub finishes processing
        return pd.DataFrame()
